...
namespace Plugin {
    namespace Sqlite {
...
#endif
                    t = ttl + Core::Time::Now().Ticks() / Core::Time::TicksPerMillisecond / 1000;
                }
                sqlite3_stmt* stmt;
...
#endif
                            t -= Core::Time::Now().Ticks() / Core::Time::TicksPerMillisecond / 1000;
                            if (t > 0) {
                                value = v;
                                ttl = t;
                                result = Core::ERROR_NONE;
        ...
        private:
            ...
            {
                TRACE(Trace::Error, (_T("%s sqlite error %d"), status));
            }
